...
export class SkyComponent implements AfterViewInit {
    ...
    initIntersectionObserver() {
        const options = {
            root: null, // Область видимости по умолчанию - вся страница
            threshold: 0 // Порог видимости элемента (0 означает, что элемент видим, когда его границы пересекают границы видимой области)
        };
        ...
        this.intersectionObserver = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    this.animate(); // Возобновляем анимацию при появлении блока в видимой области
                } else {
    ...
    animate() {
        if (!this.isIntersecting) {
            return; // Если блок не видим, прерываем анимацию
        }
        if (this.ship) {
